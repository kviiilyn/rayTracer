   ...
   (lambda ([ray : Ray] [pt : Float3] [norm : Float3])
     (local
       {(define Q : Float3 (random-point-in-sphere))
        (define S : Float3 (fl3+ norm Q))}
       (make-reflect-info rgb (make-ray pt S))))))
...
(define (metal-material rgb fuzz)
  (Material
   no-emission
   (lambda ([ray : Ray] [pt : Float3] [N : Float3])
     (local
       {(define D : Float3 (Ray-dir ray))
        (define R : Float3 (fl3-reflect D N))
        (define Q : Float3 (fl3-scale fuzz (random-point-in-sphere)))
        (define S : Float3 (fl3+ R Q))
        (define N*S : Float (fl3-dot N S))}
       (if
        (<= N*S 0)
        'None
        (make-reflect-info rgb (make-ray pt S)))))))
...
;; return rgb from its emit function
;; scatter function returns 'None
(define (diffuse-emissive-material rgb)
  (Material
   (lambda ([r : Ray] [norm : Float3]) rgb)
   (lambda ([r : Ray] [pt : Float3] [norm : Float3]) 'None)))
